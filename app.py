import os
  ...
  def get_leads():
      ...
      if search:
          query += ' AND (business_name LIKE ? OR city LIKE ? OR
  business_type LIKE ?)'
  ...
  def get_stats():
      ...
      p1 = db.execute('SELECT COUNT(*) FROM leads WHERE priority =
  1').fetchone()[0]
      p2 = db.execute('SELECT COUNT(*) FROM leads WHERE priority =
  2').fetchone()[0]
      p3 = db.execute('SELECT COUNT(*) FROM leads WHERE priority =
  3').fetchone()[0]
      contacted = db.execute('SELECT COUNT(*) FROM leads WHERE contacted =
  1').fetchone()[0]
      return jsonify({'total': total, 'p1': p1, 'p2': p2, 'p3': p3,
  'contacted': contacted})
  ...
  def update_lead(lead_id):
      ...
      updates = []
      params = []
      allowed = ['contacted', 'notes', 'dm_generated', 'priority',
  'instagram', 'phone', 'website', 'ad_status']
      ...
      db.execute(f'UPDATE leads SET {", ".join(updates)} WHERE id = ?',
  params)
      db.commit()
      return jsonify({'success': True})
  ...
  [lead_id]).fetchone()
      ...
      lead = dict(lead)
      ad_context = {
          'running_ads': 'they are currently running paid ads (Meta,
  Google, or other platforms)',
          'organic_only': 'they have strong organic social presence but no
  visible paid ads',
          'no_presence': 'they have little to no online presence',
          'unknown': 'their ad status is unknown'
      }.get(lead['ad_status'], 'unknown')
      priority_context = {
          1: 'high priority - already investing in paid advertising,
  understands ad spend',
          2: 'solid organic presence - natural next step toward paid ads',
          3: 'low digital presence - needs full buildout'
      }.get(lead['priority'], '')
      prompt = f"""Write a personalized cold Instagram DM for this
  business.
  ...
  - Max 3-4 lines total
  - Casual fun opener that sounds like a real person stumbled on their page
  not a marketer
  ...
  - Never use em-dashes lists of three things in a row the word imagine Q&A
  format or multiple exclamation marks
  - No robotic grammar or formal sentence structure
  - Write like you are texting someone casually not writing copy
  ...
  Return only the DM text. No quotes no labels nothing else."""
      ...
      db.execute('UPDATE leads SET dm_generated = ? WHERE id = ?',
  [dm_text, lead_id])
      db.commit()
  ...
  auto_reasons.get(priority, '')
      try:
          db.execute('''
              INSERT INTO leads (business_name, business_type, phone,
  website, instagram,
                                 address, city, rating, review_count,
  priority, priority_reason, ad_status)
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          ''', [
              data.get('business_name'), data.get('business_type'),
  data.get('phone'),
              data.get('website'), data.get('instagram'),
  data.get('address'),
              data.get('city'), data.get('rating'),
  ...
  auto_reasons.get(priority, '')
          try:
              db.execute('''
                  INSERT OR IGNORE INTO leads (business_name,
  business_type, phone, website, instagram,
                                 address, city, rating, review_count,
  priority, priority_reason, ad_status)
                  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
              ''', [
  ...
  lead.get('address'),
                  lead.get('city'), lead.get('rating'),
  lead.get('review_count'),
                  priority, priority_reason, lead.get('ad_status',
  'unknown')
              ])
